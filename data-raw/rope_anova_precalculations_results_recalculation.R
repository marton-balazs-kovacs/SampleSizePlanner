...
rope_anova_data_original <-
  readRDS(here("data/recalculation/ssp_rope_anova_res.rds"))

# Clean the data
rope_anova_data <-
  tibble(
    calculation_id = seq_along(rope_anova_data_original),
    parameters = map(rope_anova_data_original, "parameters"),
    output = map(rope_anova_data_original, "output")
  ) |>
